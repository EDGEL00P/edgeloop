...
function parseFeedConfig(raw) {
    ...
    try {
        if (raw.trim().startsWith('[')) {
            ...
            const feeds = parsed.map(coerceFeed).filter(Boolean);
    ...
    const tokens = raw
        ...
        .filter(Boolean);
    const feeds = tokens.map(coerceFeed).filter(Boolean);
    return feeds.length > 0 ? feeds : DEFAULT_FEEDS;
}
...
export async function getNflNews(limit = 40) {
