...
const CONTROL_ROOM_CSP = "default-src 'self'; base-uri 'none'; frame-ancestors 'none'; img-src 'self' data:; " +
...
function writeBytes(res, statusCode, body, contentType, requestId) {
    res.statusCode = statusCode;
...
function writeRedirect(res, statusCode, location, requestId) {
    res.statusCode = statusCode;
...
export function createBroadcastHttpServer(ctx) {
    ...
    function tryServeControlRoomAsset(res, pathname, method, requestId) {
        ...
        try {
            ...
            writeJson(res, 404, errorEnvelope('not_found', 'Control Room asset not built (run pnpm run build)', requestId), requestId);
        }
        return true;
    }
    const server = createServer((req, res) => {
        ...
        try {
            if (tryServeControlRoomAsset(res, path, method, requestId))
                return;
            if (path === '/healthz') {
                if (method !== 'GET') {
                    res.setHeader('allow', 'GET');
                    writeJson(res, 405, errorEnvelope('method_not_allowed', 'Method Not Allowed', requestId), requestId);
                    return;
                }
                writeJson(res, 200, {
                    status: 'ok',
            ...
            writeJson(res, 500, errorEnvelope('internal_error', 'Internal Server Error', requestId), requestId);
        }
        finally {
