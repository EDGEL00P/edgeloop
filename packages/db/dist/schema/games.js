...
export const gameStatus = ['scheduled', 'pregame', 'in_progress', 'halftime', 'final', 'postponed', 'cancelled'];
export const gameType = ['preseason', 'regular', 'wildcard', 'divisional', 'conference', 'superbowl'];
export const games = pgTable('games', {
    id: uuid('id').defaultRandom().primaryKey(),
    externalId: varchar('external_id', { length: 100 }).unique(),
    season: integer('season').notNull(),
    week: integer('week').notNull(),
    gameType: varchar('game_type', { length: 20 }).notNull().$type(),
    ...
    status: varchar('status', { length: 20 }).notNull().default('scheduled').$type(),
